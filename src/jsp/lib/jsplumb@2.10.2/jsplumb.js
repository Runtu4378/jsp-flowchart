  ...
  var jsPlumbInstance = root.jsPlumbInstance = function (_defaults) {
    ...
    this.setContainer = function (c) {
      ...
      var _oneDelegateHandler = function (id, e, componentType) {
        ...
        var jp = (t && t.parentNode ? t.parentNode._jsPlumb : null) ||
        (t ? t._jsPlumb : null) ||
    ...
    */
    ...
    var _prepareConnectionParams = function (params, referenceParams) {
      ...
      if (
        _p.sourceEndpoint &&
        _p.sourceEndpoint.connectorOverlays
      ) {
    ...
    var _finaliseConnection = _currentInstance.finaliseConnection = function (jpc, params, originalEvent, doInformAnchorManager) {
      ...
      // fire an event
      if (
        !params.doNotFireConnectionEvent &&
        params.fireEvent !== false
      ) {
        var eventArgs = {
          connection: jpc,
          source: jpc.source,
    ...
    this.getConnections = function (options, flat) {
      ...
      for (var j = 0, jj = connections.length; j < jj; j++) {
        var c = connections[j];
        var sourceId = c.proxies && c.proxies[0]
          ? c.proxies[0].originalEp.elementId
          : c.sourceId;
        var targetId = c.proxies && c.proxies[1]
    ...
    var _makeCommonSelectHandler = function (list, executor) {
      ...
      var setters = [
        'setHover', 'removeAllOverlays', 'setLabel', 'addClass', 'addOverlay', 'removeOverlay',
      ...
      if (specimen.compute && specimen.getOrientation) {
        ...
        if (_ju.isArray(specimen[0]) || _ju.isString(specimen[0])) {
          ...
          var anchorParams = {
            x: specimen[0],
            y: specimen[1],
            orientation: (specimen.length >= 4)
              ? [ specimen[2], specimen[3] ]
              : [0, 0],
            offsets: (specimen.length >= 6)
              ? [ specimen[4], specimen[5] ]
              : [ 0, 0 ],
            elementId: elementId,
            jsPlumbInstance: _currentInstance,
            cssClass: specimen.length === 7
              ? specimen[6]
              : null
          };
      ...
      var _drop = p._jsPlumb.EndpointDropHandler({
        ...
        getEndpoint: function (jpc) {
          ...
          if (newEndpoint.anchor.positionFinder != null) {
            ...
            var ap = dropPosition == null
              ? [0, 0]
              : newEndpoint.anchor.positionFinder(dropPosition, elPosition, elSize, newEndpoint.anchor.constructorParams);
